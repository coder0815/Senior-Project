...
public class TargetedFireBlast : MonoBehaviour {

    public GameObject ability;
    public Texture2D targetReticle;
    private Change_TargetedFireBlast_Cursor cursor;
    ...
    void Start()
    {
        cursor = GetComponent<Change_TargetedFireBlast_Cursor>();
    }
        ...
        if (isReady && offCoolDown)
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(ray, out hit, 100))
                {
                    ...
                    Instantiate(ability, hit.point, transform.rotation);
                    ChangeReadyState();
                    cursor.ChangeCursorBack();
                    cursor.changeCancelActive();
                    Destroy(GameObject.FindGameObjectWithTag("TargetedFireBlast"), 2.75f);
